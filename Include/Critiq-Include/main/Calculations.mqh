...
double CalculateLotSize(double risk, double slPrice) {
   double lotStep = SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_STEP);
   double minLot = SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MIN);
   double maxLot =  SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MAX);
   double tickVal = SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_VALUE);
   double accountBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   double lotSize = accountBalance * risk / 100 / (slPrice / _Point * tickVal);
...
double GetTPprice(double priceDelta, int orderType, double posRatio) {
   double price = .0;
   priceDelta = priceDelta * posRatio;
